...
/**
 * Table class for config_options
 */
class config_options extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'config_options';
		$this->TableName = 'config_options';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`config_options`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new DbField('config_options', 'config_options', 'x_id', 'id', '`id`', '`id`', 3, 11, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// parent_id
		$this->parent_id = new DbField('config_options', 'config_options', 'x_parent_id', 'parent_id', '`parent_id`', '`parent_id`', 3, 11, -1, FALSE, '`parent_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// role
		$this->role = new DbField('config_options', 'config_options', 'x_role', 'role', '`role`', '`role`', 200, 255, -1, FALSE, '`role`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// name
		$this->name = new DbField('config_options', 'config_options', 'x_name', 'name', '`name`', '`name`', 200, 255, -1, FALSE, '`name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// value
		$this->value = new DbField('config_options', 'config_options', 'x_value', 'value', '`value`', '`value`', 201, 65535, -1, FALSE, '`value`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`config_options`";
	}
	...
	{
		global $Language;
		if ($pageName == "config_optionsview.php")
			return $Language->phrase("View");
		elseif ($pageName == "config_optionsedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "config_optionsadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("config_optionsview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("config_optionsview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "config_optionsadd.php?" . $this->getUrlParm($parm);
		else
			$url = "config_optionsadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("config_optionsedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("config_optionsadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("config_optionsdelete.php", $this->getUrlParm());
	}
