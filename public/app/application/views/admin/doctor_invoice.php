...
<head>
	...
	<title>Doctor Invoice - Lims</title>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                          <div class="">
                                    ...
                                    <form method='get'>
                                        ...
                                        <div class="col-lg-4">
                                            <div class="form-group">
                                                ...
                                                <a href="<?php echo base_url();?>admin/doctorInvoice" class="mt-2 btn btn-outline-danger btn-theme">Reset</a>
                                            </div>
                                        </div>
                                    </form>
                                </div>
                            ...
                            <div class="card">
                                <div class="card-content">
                                  <span class="text-theme  bootstrap-table">Doctor Invoices</span>
                                    <div class="toolbar">
                                        <!--Here you can write extra buttons/actions for the toolbar-->
                                    </div>
                                    <table id="bootstrap-table" class="table-striped">
                                        <thead>
                                            <th data-field="invoice" class="td-actions text-left" >Invoice #</th>
                                            <th data-field="paid" data-sortable="true" class="td-actions text-left" >Paid</th>
                                        ...
                                        <?php foreach($jobDetails as $job){ ?>
											...
											<td><a target="_blank" class="btn btn-outline-danger btn-theme" href="<?php echo base_url();?>doctor/viewInvoice/<?php echo $job->test_id?>">
                                                    View Invoice
                                                </a>
                                            </td>
                                            <td class="text-center">
											
											<?php if($job->doctor_paid == 0){?>
											<!--<a class="btn btn-outline-danger btn-theme" href="<?php echo base_url();?>admin/payInvoice/<?php echo $job->test_id?>">
                                                    Pay Invoice
                                                </a>-->
											...
											<?php }elseif($job->doctor_paid == 1){?>
												<!-- <a  target="_blank" class="btn btn-outline-danger btn-theme" href="<?php echo base_url();?>doctor/viewInvoice/<?php echo $job->test_id?>">
													Paid
												</a>-->
											...
											<?php }?>
                                                ...
                                                <td><a class="profile_link" href="<?php echo base_url();?>doctor/DoctorDetails/<?php echo $job->doctor_id?>"> <?php echo 'Dr. '.$job->doctor_name;?></a></td>
												                        <td><a class="profile_link" href="<?php echo base_url();?>hospital/HospitalDetails/<?php echo $job->hospital_id?>"><?php echo $job->hospital_name;?></td>
                                                  <td>
                                                   <?php if(!empty($job->pathologists_fee)){ echo "£".$job->pathologists_fee ;}else{ echo 'N/A' ;}?>
                                                </td>
                                                <td><?php $Idate=date_create($job->invoice_date); echo date_format($Idate,$dateFormat); ?></td>
                           ...
                           <?php if(!empty($this->input->get('startdate'))){
                             ...
                             <a href="<?php echo base_url();?>admin/AllDoctorInvoiceexport<?php echo $startdate.$enddate; ?>" class="btn btn-outline-danger btn-theme mb-5 btn-wd">Export Invoice CSV</a>
                        </div> <!-- end col-md-12 -->
                    </div> <!-- end row -->
                </div>
            </div>
    ...
    $('body').on('change', '.payInvoice', function() { 
        if($(this).is(":checked")){
            ...
            var retVal = confirm("By clicking Paid - An email will be sent to confirm that this Invoice has been paid");
               if( retVal == true ) {
                   $.ajax({url: "<?php echo base_url();?>admin/payInvoice/"+postId, success: function(result){
                        location.reload();                    
                    } 
 ...
 $(document).ready(function(){
   $('.unpayInvoice').change(function(){ 
        if($(this).is(":unchecked")){
           ...
           // var retVal = confirm("By clicking Paid - An email will be sent to confirm that this Invoice has been paid");
              // if( retVal == true ) {
                   $.ajax({url: "<?php echo base_url();?>admin/UnpayInvoice/"+postId, success: function(result){
                        location.reload();                    
                    } 
